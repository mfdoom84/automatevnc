...
function App() {
    ...
    // Handle script deletion (called from ProjectExplorer)
    const handleDeleteScript = useCallback((name) => {
        // Remove from memory
        delete codeByScriptRef.current[name]

        // Remove from tabs
        setOpenScripts(prev => prev.filter(s => s !== name))
        setDirtyScripts(prev => prev.filter(s => s !== name))

        // If it was the current script, clear view
        if (currentScript?.metadata.name === name) {
            setCurrentScript(null)
            setCode('')
            // Optional: fallback to first open tab
            const remaining = openScripts.filter(s => s !== name)
            if (remaining.length > 0) {
                loadScript(remaining[0])
            }
        }
    }, [currentScript, openScripts, loadScript])
